...
def parse_args():
    ...
    parser.add_argument('--config', help='test config file path', default='/home/sidd_s/scratch/mic_checkpoints/csHR2dzurHR_mic_hrda_aef07/csHR2dzurHR_mic_hrda_aef07.py')
    parser.add_argument('--checkpoint', help='checkpoint file', default='/home/sidd_s/scratch/mic_checkpoints/csHR2dzurHR_mic_hrda_aef07/iter_40000_relevant.pth')
    parser.add_argument(
...
def main():
    ...
    if args.test_set:
        for k in cfg.data.test:
            if isinstance(cfg.data.test[k], str):
                # cfg.data.test[k] = cfg.data.test[k].replace('val', 'test')  # can commment for making pred for val dataset of DZ
                pass 
    ...
    if not distributed:
        ...
        # print('>>>>>>>>>>>', len(outputs), outputs[0], outputs[0].shape) # labels trainids of same shape as that of image
        # print('*******', outputs) ## issue for now, is that outputs is coming empty list...which should be resolved :), it is resolved!! 
    else:
    ...
    if rank == 0:
        ...
        if args.eval:
            cfg_img_scale = cfg.data.test['pipeline'][1]['img_scale']
            res = dataset.evaluate(outputs, cfg_img_scale, args.eval, **kwargs)
